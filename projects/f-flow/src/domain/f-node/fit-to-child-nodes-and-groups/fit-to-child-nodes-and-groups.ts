...
import { FitChildrenToChildNodesRequest } from '../fit-children-to-child-nodes';
...
export class FitToChildNodesAndGroups implements IExecution<FitToChildNodesAndGroupsRequest, void> {
  ...
  public handle({ nodeOrGroup }: FitToChildNodesAndGroupsRequest): void {
    // If fChildren directive is present, delegate to FitChildrenToChildNodes
    if (this._hasFChildren(nodeOrGroup)) {
      this._mediator.execute<void>(new FitChildrenToChildNodesRequest(nodeOrGroup));
  ...
  private _hasFChildren(nodeOrGroup: FNodeBase): boolean {
    if (!this._browser.isBrowser()) {
      return false;
    }

    return nodeOrGroup.hostElement.querySelector(`[data-f-children]`) !== null;
  }
